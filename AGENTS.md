...
- 仓库根目录 profile bundles（可直接载入 `Local_Lab/profile_dashboard.html`）：
  ...
  - **当前 main score reference**：`profile_bundle_logs/cache-predictor-c4-transport-halves-4n64-16ppn_20260809T041907Z_profile_bundle.json`
...
- **reference 规则**：每个新 accepted commit 的 score DEMO run 直接成为下一项实验的 `--reference-run`。当前 reference 为 `Local_Lab/runs/profile128/cache-predictor-c4-transport-halves-4n64-16ppn_20260809T041907Z_20538`（job `118811614`，profile total `70.80s`，26 变量逐位一致；binary SHA-256 `f571ff75f9200663ea07e9d966c5dc455cd7364f57de3898f25291aff8ab757c`）。相对此前 reference，缓存 predictor C4 half-transport 平面使 R22 mean 在 Grid 1/2 下降 `0.43/1.31%`，调用次数不变；同一 allocation 的 R35/R39/R49/R55 普遍变慢，故 total 增加 `0.63s` 属于运行噪声，R22 逆势下降构成局部因果证据。上一项 predictor 全湿 mask 的独立 validate job `118811117` 已通过。reference 的首要作用仍是输出基准和 region 对照，不把某一次 wall 当成无误差真值。旧的 2 节点、4 节点 128-rank 和早期 4n64 references 仅作历史对照。
