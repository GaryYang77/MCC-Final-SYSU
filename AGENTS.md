...
- 仓库根目录 profile bundles（可直接载入 `Local_Lab/profile_dashboard.html`）：
  ...
  - **当前 main score reference**：`profile_bundle_logs/batch-step2d-zeta-halos-4n64-16ppn_20260809T000452Z_profile_bundle.json`
...
- **reference 规则**：每个新 accepted commit 的 score DEMO run 直接成为下一项实验的 `--reference-run`。当前 reference 为 `Local_Lab/runs/profile128/batch-step2d-zeta-halos-4n64-16ppn_20260809T000452Z_12870`（job `118806901`，profile total `71.10s`，26 变量逐位一致；binary SHA-256 `201dbb458a14796d9cdaec80f60169e5d9eb13b585d76be4bdacbb5a5c88c7d5`）。相对此前 `71.20s` reference，predictor `rzeta/zeta` 合包使 R40 调用数双网格下降 `8.39%`，R40 mean 在 Grid 1/2 下降 `0.98/4.31%`；独立 1-rank validate job `118806937` 通过。单次 total 受节点噪声影响；reference 的首要作用是输出基准和 region 对照，不把某一次 wall 当成无误差真值。旧的 2 节点、4 节点 128-rank 和早期 4n64 references 仅作历史对照。
