...
- R35 horizontal tracer advection 和 R22 `pre_step3d` 已完成第一轮计算优化；
  当前第一计算目标是 Grid-2 R09 `step2d`。R09 已拆成八个阶段，transport/setup
  是最大子段，其次是 advection/rotation 和 viscosity；下一步先区分 transport/setup
  中的本地计算与通信，再按新 score 重排 R19 GLS、R34 `step3d_uv` 和 halo。
...
- profiler-v2（commit `64cec19`，2026-08-08）已通过 Phase-D：score、summary、trace 三种用途分离；summary 可细分 contact/f2csum 的 plan-pack-MPI-unpack、tracer corrector、R22 `pre_step3d` 七个子阶段、R09 `step2d` 八个子阶段、broadcast 和 `put_refine3d`，并记录 rank/node；trace 可为选定 ranks 离线生成 Perfetto JSON。普通 score build 不含 `PROFILE_DIAGNOSTIC`，新增诊断不会进入最终 no-profile 二进制。R35、R22 与 R09 已达到子阶段粒度，R34 仍是宽 region；halo2d/3d/4d 的 pack/wait/unpack 名称已注册但源码尚未打桩，不得误称已有分解结果。详见 `Local_Lab/profiler-v2-design.md`、`Local_Lab/profiler-v2-phase-d.md` 和 Phase 6 计划。
