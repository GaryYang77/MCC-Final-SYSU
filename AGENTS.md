...
- 仓库根目录 profile bundles（可直接载入 `Local_Lab/profile_dashboard.html`）：
  ...
  - **当前 main score reference**：`profile_bundle_logs/gls-kkl-boundary-powers-4n64-16ppn_20260809T005828Z_profile_bundle.json`
...
- **reference 规则**：每个新 accepted commit 的 score DEMO run 直接成为下一项实验的 `--reference-run`。当前 reference 为 `Local_Lab/runs/profile128/gls-kkl-boundary-powers-4n64-16ppn_20260809T005828Z_6875`（job `118807778`，profile total `71.70s`，26 变量逐位一致；binary SHA-256 `98e6c7954f86cb361e35ef8372aabb473f8589460a9d0684d2e80f97c3077c2f`）。相对此前 `71.10s` reference，K-kl 边界单位/零次幂特化使 R19 mean 在 Grid 1/2 下降 `4.80/2.98%`，调用次数不变；独立 1-rank validate job `118807823` 通过。该次 total 被 R44 I/O 等待增加所掩盖；reference 的首要作用是输出基准和 region 对照，不把某一次 wall 当成无误差真值。旧的 2 节点、4 节点 128-rank 和早期 4n64 references 仅作历史对照。
